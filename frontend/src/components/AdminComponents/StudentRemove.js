...
const StudentRemovePage=()=>{
    const location = useLocation();
     ...
     useEffect(()=>{
       ...
       const getlist = async ()=>  {
                    const AdminDataResponse = await axios.post('http://localhost:5000/api/StudentRoute/getStudentDetails',{dept : deptval,section : secVal,year : ayear});
     ...
     const handleDeleteClick = (userId) => {
      ...
      const deleteUser = async () => {
        try {
         ...
         const response= await axios.post('http://localhost:5000/api/StudentRoute/removeStudent',{id:adminIdRemove}); // Assuming your API endpoint is /api/users/:id
    ...
    return (<div>
   <Kmithead></Kmithead>
                <div className=" h-10 flex justify-center items-center bg-red-200 m-2"> <h1 className="text-xl font-semibold"> Student Removal Section</h1> </div>
                <div className="m-2 flex justify-center items-center bg-red-200"> <h1 className="m-1 text-lg font-semibold">{deptval} </h1> <h1 className="m-1 text-lg font-semibold">{ayear} Year</h1></div>
      ...
      {showAlert && adminIdRemove != null  && (
        <div className="confirmation-dialog justify-center">
         ...
         <div className="flex justify-center"> <button onClick={handleConfirmDelete} className="p-1 bg-red-300 rounded-md m-1">Confirm</button>
          <button onClick={handleCancelDelete} className="p-1 bg-green-200 rounded-md m-1">Cancel</button>
          </div>
        </div>
      )}
       </div>
        ...
        { (AdminList !== null && AdminList.length !== 0) ? (AdminList.map((Admin)=>(
          <div className="flex flex-col-3 h-10 m-2 p- justify-evenly items-center border border-red-400"> 
            ...
            <div><button className="bg-red-300 p-1 m-1 rounded-md" onClick={()=>{handleDeleteClick(Admin.roll_no)}}>Remove</button></div>
            </div>
