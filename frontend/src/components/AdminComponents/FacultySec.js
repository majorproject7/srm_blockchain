...
function FacultyManage() {
  return (
    <>
      ...
      <div className="p-5 mx-10 h-screen flex flex-row justify-center">
        <div className="m-2">{Branchcomponent()}</div>
        <div className="m-2 ">{buttonPanel()} </div>
      </div>
    </>
  );
}
    ...
    if (deptval !== null) {
      const getfaculty = async () => {
        const response = await axios.post(
          "http://localhost:5000/api/TeacherRoute/getBranchFaculty",
          { dept: deptval }
        );
